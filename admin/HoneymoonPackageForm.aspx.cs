...
public partial class admin_HoneymoonPackageForm : System.Web.UI.Page
{
    ...
    {
        con.Open();
        SqlCommand cmd = new SqlCommand("SELECT * FROM honeymoonpackageForm", con);
    ...
    {
        con.Open();
        SqlCommand cmd = new SqlCommand("INSERT INTO honeymoonpackageForm (place_id, place_name ,packageDuration, packageAvailableTickets, packageStartDate, packageEndDate, packageInclusions ,status, rts) VALUES ('" + ddl_selectplace.SelectedValue + "','" + ddl_selectplace.SelectedItem + "','" + txtpackageDuration.Text + "','" + txtpackageAvailableTickets.Text + "','" + txtpackageStartDate.Text + "','" + txtpackageEndDate.Text + "','" + txtpackageInclusions.Text + "','1',getdate())", con);
        cmd.ExecuteNonQuery();
        cmd.Dispose();
        con.Close();
        clear();
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        SqlCommand cmd = new SqlCommand("Update honeymoonpackageForm Set place_id='" + ddl_selectplace.SelectedValue + "',packageAvailableTickets='" + txtpackageAvailableTickets.Text + "', packageStartDate='" + txtpackageStartDate.Text + "', packageEndDate='" + txtpackageEndDate.Text + "',packageInclusions='" + txtpackageInclusions.Text + "', status='1' where Id='" + Idd + "'", con);
        cmd.ExecuteNonQuery();
        con.Close();
        BindListView();
        clear();
    ...
    protected void ListView1_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "delete")
        {
            Label lblid = (Label)e.Item.FindControl("lblid");
            con.Open();
            SqlCommand cmd = new SqlCommand("Delete From honeymoonpackageForm Where id='" + lblid.Text + "'", con);
            cmd.ExecuteNonQuery();
            cmd.Dispose();
            con.Close();
            BindListView();
        ...
        if (e.CommandName == "Active")
        {
            Label lblid = (Label)e.Item.FindControl("lblid");
            con.Open();
            SqlCommand cmd = new SqlCommand("Update honeymoonpackageForm Set status='1' where id='" + lblid.Text + "'", con);
            cmd.ExecuteNonQuery();
            cmd.Dispose();
            con.Close();
            BindListView();
        ...
        if (e.CommandName == "Deactive")
        {
            Label lblid = (Label)e.Item.FindControl("lblid");
            con.Open();
            SqlCommand cmd = new SqlCommand("Update honeymoonpackageFormSet status='0' where id='" + lblid.Text + "'", con);
            cmd.ExecuteNonQuery();
            cmd.Dispose();
            con.Close();
            BindListView();
